...
def run(input_data: dict, solver_params: dict, extra_arguments: dict) -> dict:
    ...
    penalty_overlap = max_cost * 4.0      # Time overlap on same berth (v3.1: increased from 1.5 to 4.0)
    ...
    # ── 9. Generate additional output visualizations ─────────────────
    try:
...
""" % (int(cost_breakdown["total_cost"]), int(cost_breakdown["cost_per_vessel"]),
       round(cost_breakdown["cost_per_teu"], 4), _json_safe(cost_breakdown), _json_safe(assignments))
...
<body>
    ...
    <div class="metrics-grid">
        <div class="metric-box">
            ...
            <div class="metric-detail">Assignment: %s, Crane: %s</div>
        </div>
        <div class="metric-box">
            <div class="metric-title">QUBO Density</div>
            <div class="metric-value">%.6f</div>
            <div class="metric-detail">Nonzero entries: %s</div>
        </div>
        <div class="metric-box">
            <div class="metric-title">Constraint Satisfaction</div>
            <div class="metric-value">%s / %s vessels</div>
            <div class="metric-detail">One-berth: %s, One-crane: %s</div>
        </div>
        <div class="metric-box">
            <div class="metric-title">Quantum Advantage</div>
            <div class="metric-value">Hardware Ready: %s</div>
            <div class="metric-detail">D-Wave Compatible: %s</div>
        </div>
        <div class="metric-box">
            <div class="metric-title">Wall Time</div>
            <div class="metric-value">%.3f seconds</div>
            <div class="metric-detail">SQA: %.3f s, Repair: %.3f s</div>
        </div>
        <div class="metric-box">
            <div class="metric-title">Algorithm</div>
            <div class="metric-value">%s</div>
            <div class="metric-detail">Trotter Slices: %s, Version: %s</div>
        </div>
    </div>
</body>
</html>
""" % (
    qubo_analysis["total_variables"],
    qubo_analysis["assignment_variables"],
    qubo_analysis["crane_variables"],
    qubo_analysis["matrix_density"],
    qubo_analysis["nonzero_entries"],
    qubo_analysis["constraint_satisfaction"]["one_berth_per_vessel"],
    qubo_analysis["constraint_satisfaction"]["total_vessels"],
    qubo_analysis["constraint_satisfaction"]["one_berth_per_vessel"],
    qubo_analysis["constraint_satisfaction"]["one_crane_per_vessel"],
    quantum_advantage["hardware_ready"],
    quantum_advantage["dwave_compatible"],
    computation_metrics["wall_time_s"],
    computation_metrics["sqa_time_s"],
    computation_metrics["repair_time_s"],
    computation_metrics["algorithm"],
    computation_metrics.get("trotter_slices", 20),
    computation_metrics["solver_version"]
)
